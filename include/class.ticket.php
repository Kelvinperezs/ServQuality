...
class Ticket{
	...
	if ($dept->autoRespONNewTicket()) {
	  ...
	  $subj= "Solicitud de ayuda #" .$this->getExtId(). " cerrada.";
	  // I added a link in the body to the ticket for the user if they wanted
	  // to view it just after I closed it.
	  $body= "Tu Ticket #" .$this->getExtId(). " sera cerrado.<br>Puede consultar la informaci&oacute;n sobre la solicitud en:<br> http://" . $_SERVER["SERVER_NAME"] . "/tts/view.php?e=" . $this->getEmail() . "&t=" . $this->getExtId() . " Ver.";
   ...
   /*============== Functions below do not require an instance of the class to be used. To call it use Ticket::function(params); ==================*/
    ...
    function update($var,&$errors) {
         ...
         $fields['email']    = array('type'=>'email',    'required'=>1, 'error'=>'Email requerido');
         $fields['note']     = array('type'=>'text',     'required'=>1, 'error'=>'Razon de la actualizaci&oacute;n requerida');
         $fields['subject']  = array('type'=>'string',   'required'=>1, 'error'=>'Asunto requerido');
         $fields['topicId']  = array('type'=>'int',      'required'=>0, 'error'=>'Selecci&oacute;n invalida');
         $fields['pri']      = array('type'=>'int',      'required'=>0, 'error'=>'Prioridad invalida');
         $fields['phone']    = array('type'=>'phone',    'required'=>0, 'error'=>'Se requiere un numero de tel&eacute;fono valido');
         $fields['duedate']  = array('type'=>'date',     'required'=>0, 'error'=>'Fecha invalida, Formato DD/MM/YY');
    ...
    function create($var,&$errors,$origin,$autorespond=true,$alertstaff=true) {
       ...
       /* Coders never code so fully and joyfully as when they do it for free  - Peter Rotich */
        ...
        if(strcasecmp($origin,'web')==0) { //Help topic only applicable on web tickets.
            $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>'Selecciona &aacute;rea de ayuda');
        }elseif(strcasecmp($origin,'staff')==0){ //tickets created by staff...e.g on callins.
            $fields['deptId']   = array('type'=>'int',      'required'=>1, 'error'=>'Departamento requerido');
            $fields['source']   = array('type'=>'string',   'required'=>1, 'error'=>'Indicar el origen');
            $fields['duedate']  = array('type'=>'date',    'required'=>0, 'error'=>'Fecha invalida, Format DD/MM/YY');
        }else { //Incoming emails
            $fields['emailId']  = array('type'=>'int',  'required'=>1, 'error'=>'Email desconocido');
        }
        $fields['pri']      = array('type'=>'int',      'required'=>0, 'error'=>'Prioridad invalida');
        $fields['phone']    = array('type'=>'phone',    'required'=>0, 'error'=>'Se requiere numero de tel&eacute;fono valido');
        ...
        if(!$errors && BanList::isbanned($var['email'])) {
            $errors['err']='Ticket denegado. Error #403 solo se permite correo institucional'; //We don't want to tell the user the real reason...Psssst.
    ...
    function create_by_staff($var,&$errors) {
        ...
        if(!$var['issue'])
            $errors['issue']='Resumen de la insidencia requererido';
