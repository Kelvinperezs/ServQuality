...
//Asegúrese de que el usuario se escriba administrador bloquear !
if(!$thisuser or !$thisuser->isadmin()){
    header('Location: index.php');
    require('index.php'); // por si acaso!
    exit;
}


//Algunas advertencias relacionadas con la seguridad!!! :)
if(defined('THIS_VERSION') && strcasecmp($cfg->getVersion(),THIS_VERSION)) {
    ...
    if(file_exists('../setup/')){
	    ...
	    if(CONFIG_FILE && file_exists(CONFIG_FILE) && is_writable(CONFIG_FILE)) {
            //Confirme de verdad que el archivo se puede escribir por el grupo.
	    clearstatcache(); //borrar la caché!
	    ...
	    if(($perms & 0x0002) || ($perms & 0x0010)) { 
		    ...
		    #basename (CONFIG_FILE), nombre base (CONFIG_FILE));
		    }
        }
...
//Acceso desprotegido bien ... permite hacer el do
define('OSTADMININC',TRUE); //comprobado por admin incluir archivos
define('ADMINPAGE',TRUE);   //Utilizado por la cabecera para cambiar los menús.

//Archivos que podríamos necesitar.
//TODO: No en Exigir requerir ... ahorrar algo de mem.
...
if($_POST && $_REQUEST['t'] && !$errors):
    ...
    $errors=array(); //hacerlo de todos modos.

    switch(strtolower($_REQUEST['t'])):
        case 'pref':
            //Hacer el trabajo sucio detrás de las escenas.
        ...
        case 'attach':
            if($_POST['allow_attachments'] or $_POST['upload_dir']) {
                ...
                if($_POST['upload_dir']) //conseguir el camino real.
            ...
            case 'mass_process':
                ...
                if($_POST['uids'] && is_array($_POST['uids'])) {
                    ...
                    if(isset($_POST['enable'])) {
                        ...
                        //mira lo que está haciendo ... ¿por qué no acaba de dejar caer el DB?
                        ...
                        //Suelte boletos asignados al usuario ?? NO? podría ser una cosa temporal 
                        // Puede ser de liberación automática si no conectado a X días? 
            ...
            case 'mass_process':
                if(!$_POST['ids'] || !is_array($_POST['ids'])) {
                    ...
                    if($_POST['public']){
                        ...
                        //Denegar todas las eliminaciones si una de las selecciones tiene miembros en ella.
                        ...
                        if($members){
                            ...
                            //Tenemos que hacer frente a la selección individual debido boletos asociados y usuarios.
                            $i=0;
                            foreach($_POST['ids'] as $k=>$v) {
...
switch($thistab){
    ...
    case 'topics':
        ...
        break;
    //Personal (usuarios, grupos y equipos)
    case 'grp':
    case 'groups':
    case 'staff':
        $group=null;
        //Opciones de la ficha y Nav.
...
//Ahora vamos a representar la página...
